...
import 'package:error_handler/extensions/text_extension.dart';
...
class _GETMethodTileState extends State<GETMethodTile> {
  Future<void> fetchUsers() async {
    await widget.dioClient
        .getUser(id: 101203032)
        .then((value) => print('hello'))
        .catchError((e) {
      print(e);
      print(json.encode(e));
      BackendException response = Cookies.decodeBackendResponse(e);
      if (response.errorCode == 400) {
        print('cerrando sesion');
      }
      print(response.errorCode);
      print(response.errorMensaje);
      print(response.messague);
    });
  }
            ...
            : FutureBuilder<User?>(
                ...
                builder: (context, snapshot) {
                  if (snapshot.connectionState == ConnectionState.waiting) {
                    ...
                    BackendException response =
                        Cookies.decodeBackendResponse(snapshot.error);
                    print(response.errorCode);
                    print(response.errorMensaje);
                    print(response.messague);
                    return Container();
